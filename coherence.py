    ...
    with tf.Graph().as_default() as graph:
        ...
        with tf.Session(config=config) as sess:
            ...
            for topic_idx in range(params.num_topics):
                ...
                generated_topic_words_cnt = Counter(
                    [word for sent in generated_sentences for word in sent]
                )
                for k in generated_topic_words_cnt:
                    if k not in topic_vocab:
                        generated_topic_words_cnt[k] = 0

                top_generated_words = set(
                    x[0] for x in generated_topic_words_cnt.most_common(10)
                )
                ...
                top_topic_words_idx = np.argsort(topic_word_dist)[-10:]
                top_topic_words = set(
                    topic_idx2word[x] for x in top_topic_words_idx
                )

                score = len(top_topic_words.intersection(top_generated_words))

...
class Dictionary(object):
    def __init__(self, sentences, val_sentences, vocab_drop):
        ...
        self._words = []
        self._vocab_list = []
        self.get_words(self._sentences)
        self.get_words(self._val_sentences)
        # add tokens
...
def prepare_data(data_raw, val_data_raw, params):
    ...
    labels = [[data_dict.word2idx[word] \
                   for word in sent[1:]] for sent in data_dict.sentences \
                  if len(sent) < params.sent_max_size - 2]
    val_labels = [[data_dict.word2idx[word] \
    ...
    return data, labels, embed_arr, data_dict, val_labels

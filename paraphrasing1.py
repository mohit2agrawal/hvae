...
def main(params):
    ...
    with tf.Graph().as_default() as graph:
        ...
        Zsent_distribution, zsent_sample_enc, Zglobal_distribition, zglobal_sample_enc, zsent_state, zglobal_state = encoder(
            vect_inputs, label_inputs_1, batch_size, max_len_word, max_len_label
        )
        ...
        with tf.Session() as sess:
            ...
            biased_sampling = True ## keep this True
            ...
            with open(out_sentence_file,
                ...
                for num in tqdm(range(number_of_samples)):
                    ...
                    # print('\ngetting z values from encoder')

                    sent_l_batch = encoder_data[[num]]
                    label_l_batch = encoder_labels[[num]]
                    ...
                    _encoder_labels = list(
                        map(
                            lambda x: label_dict.idx2word[x], label_l_batch[0]
                        )
                    )
                    label_f.write(
                        ' '.join(
                            _encoder_labels[:_encoder_labels.index(label_dict.eos)]
                        )
                    )
                    label_f.write('\n')
                    ...
                    for para_num in range(PARA_NUM):
                        ...
                        pred_label_idx = label_dict.word2idx[data_dict.bos]
                        ...
                        while True:
                            ...
                            if biased_sampling and label_sent_len == 0:

        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(params.num_epochs):
                ...
                for it in tqdm(range(num_iters)):
                    ...
                    #     feed_dict=feed
                    # )
                    tlb, _, rl, lrl, wrl = sess.run(
                        [
                            total_lower_bound, optimize, rec_loss,
                            label_rec_loss, word_rec_loss
                        ],
                        feed_dict=feed
                    )
                    ...
                    # )
                    write_lists_to_file(
                        'test_plot.txt', all_alpha, all_beta, all_tlb, all_rl,
                        all_lrl, all_wrl
                    )

                    # if len(all_tlb) > 312 * 2:
                    #     prev1 = np.mean(all_tlb[-312:])
                    #     prev2 = np.mean(all_tlb[-312 * 2:-312])
                    #     if abs(prev2 - prev1) / prev2 < 0.01:
                    #         finish_epoch_loop = True
                    #         break
